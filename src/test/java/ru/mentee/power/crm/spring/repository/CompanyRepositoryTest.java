...
class CompanyRepositoryTest {
	...
	void shouldSaveCompanyWithLeads() {
		...
		assertThat(saved).extracting(Company::getLeads).asList().hasSize(3);

		Company found = companyRepository.findByIdWithLeads(saved.getId()).orElseThrow();

		assertThat(found).extracting(Company::getLeads).asList().hasSize(3);
	}
	...
	void shouldAvoidN1WithEntityGraph() {
		...
		assertThat(found).extracting(Company::getLeads).asList().hasSize(5);
	}
}
